...
Start-Process pwsh -ArgumentList "-NoExit", "-Command", "Set-Location '$root\aether\contracts'; npx hardhat node"
...
if ($contractLine) {
    ...
    Write-Host "✅ Updated backend/.env with CONTRACT_ADDRESS" -ForegroundColor Green
...
"@ | Set-Content $frontendEnv
    ...
    Write-Host "⚠️  Could not extract contract address — check deploy output above." -ForegroundColor Red
}

# ─── Step 4: Start Backend ─────────────────────────────────────────────────────
Write-Host "⚙️  Starting Backend (http://localhost:3001)..." -ForegroundColor Yellow
Start-Process pwsh -ArgumentList "-NoExit", "-Command", "Set-Location '$root\aether\backend'; npm run dev"

# ─── Step 5: Start Frontend ────────────────────────────────────────────────────
Write-Host "🌐 Starting Frontend (http://localhost:3000)..." -ForegroundColor Yellow
Start-Process pwsh -ArgumentList "-NoExit", "-Command", "Set-Location '$root\aether\frontend'; npm run dev"
